...
class MainScreen(Screen):
    ...
    def swap_digits(self, value):
        s = f"{value:02}"
        return int(s[1] + s[0])

    def normalize_time(self, h, m, s):
        if s >= 60:
            m += s // 60
            s = s % 60
        if m >= 60:
    ...
    def calculate_times(self, instance):
        ...
        step4_times = [
            step4_row1.strftime("%H:%M:%S"),
            f"{step4_row2_h:02}:{step4_row2_m:02}:{step4_row2_s:02}",
            f"{step4_row3_h:02}:{step4_row3_m:02}:{step4_row3_s:02}",
            f"{step4_row4_h:02}:{step4_row4_m:02}:{step4_row4_s:02}"
        ]
